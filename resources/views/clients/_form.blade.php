    @component('components.well')
        @slot('type') clients @endslot
        @if (isset($client))
          {{ Form::model($client, ['route' => ['clients.update', $client->id], 'method' => 'put', 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data']) }}
        @else
          {{ Form::open(['url' => route('clients.store'), 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data']) }}
        @endif
        <fieldset>
            
            @if ($formType == 'edit')
              <legend>Редактирование клиент</legend>
            @else
              <legend>Добавление нового клиента</legend>
            @endif
            <div class="form-group {{ $errors->has('name') ?  'has-error' : ''}}">
                {{ Form::label('name', 'Название клиента', ['class' => 'col-md-3 control-label']) }}
                <div class="col-md-9">
